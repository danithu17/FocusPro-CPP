    ...
    while (window.isOpen()) {
        while (const std::optional event = window.pollEvent()) {
            ...
            if (const auto* mb = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                // Start button
                if (sf::FloatRect({500.f, 350.f}, {180.f, 50.f}).contains(mPos)) isRunning = !isRunning;
            }
        }
        ...
        // 2. DRAW PLAYER (Apple Music Style)
        ...
        // 4. MAIN BUTTON
        sf::RectangleShape btn({180.f, 50.f}, sf::Vector2f(25.f, 25.f)); // Rounded (conceptual)
        ...
        btText.setPosition({545.f, 362.f});
        window.draw(btText);
